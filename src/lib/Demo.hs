...
envHandler3 =
  castHandler (opsCast cast) $
    composeExactHandlers
      envHandler1
      (castHandler (opsCast cast) envHandler2
        :: Handler (Union (EnvEff Int) NoEff) (EnvEff Int) eff eff)
...
readerComp5 = castComputation (opsCast cast) $
  bindExactHandler envHandler1
    (castComputation (opsCast cast) readerComp4
      :: EffectfulComputation (Union (EnvEff Int) NoEff) Int Identity)
